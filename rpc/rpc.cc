	...
	while (1) {
...

		{
			...
			while (!ca.done) {
				...
				if (pthread_cond_timedwait(&ca.c, &ca.m, &nextdeadline) == ETIMEDOUT) {//if timeout no wait
		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	for(it=reply_window_[clt_nonce].begin();it!=reply_window_[clt_nonce].end();it++){
		if ((*it).xid==xid ) {
			//duplicate
			if ((*it).cb_present==false) {//check any data on the duplicate
				return INPROGRESS;
			}			
			*b=(*it).buf;
			*sz=(*it).sz;
	...
	//put a dummy reply_t in the window, the xid in reply_window should be in order
	reply_t rpt(xid);
	rpt.cb_present=false;
	for (it=reply_window_[clt_nonce].begin(); it!=reply_window_[clt_nonce].end(); it++) {
		if ((*it).xid>xid) {
	...
	reply_window_[clt_nonce].push_back(rpt);
	
gg:
	if (reply_window_[clt_nonce].back().xid<xid_rep) {
		return FORGOTTEN;
	}
