...
config::paxos_commit(unsigned instance, std::string value)
{
  ...
  newmem = members(value);  //add value to new members
  ...
  if (vc) {
    assert(pthread_mutex_unlock(&cfg_mutex)==0);
    vc->commit_change();       //if vc doesnt return it will hang on 
    assert(pthread_mutex_lock(&cfg_mutex)==0);
  }
  assert(pthread_mutex_unlock(&cfg_mutex)==0);
}
...
config::add(std::string new_m)
{
  ...
  m = mems;
  m.push_back(new_m);
  std::string v = value(m);   //the new one 
  assert(pthread_mutex_unlock(&cfg_mutex)==0);
...
config::remove_wo(std::string m)
{
  ...
  for (unsigned i = 0; i < mems.size(); i++) {  //remove m 
...
config::heartbeater()
{
  ...
  retry:
    ...
    pthread_cond_timedwait(&config_cond, &cfg_mutex, &next_timeout);
	  ...
	  //before fire heartbeat, check if the timer has been around for 3 sec
	  gettimeofday(&now, NULL);
	  if (now.tv_sec - commit_time.tv_sec <= 3) { //less than 3 sec
		  goto retry;
	  }
