...
retry2:
	//delete the primary from the mems, chose a new replica as an new primary
	for (std::vector<std::string>::iterator it = known_mems.begin(); it != known_mems.end(); it ++) {
		if ((*it).compare(primary.id) == 0) {  //find the primary
			known_mems.erase(it); //delete the old primary
			goto retry2;
		}
	}
	
	// set the new primary   
	std::string newprimary = known_mems.front();   // chose the front 
	
	sockaddr_in dstsock;
	make_sockaddr(newprimary.c_str(), &dstsock);
	primary.id = newprimary;
	if (primary.cl) {
